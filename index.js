...
const activePurchases = new Map(); // تتبع مراحل الشراء المتقدمة

// أسعار الصرف التقريبية للتحويل إلى الدولار ($)
const exchangeRates = {
    'ريال': 0.27,    // الريال السعودي تقريباً 0.27 دولار
    'sar': 0.27,
    'درهم': 0.27,
    'aed': 0.27,
    'يورو': 1.08,    // اليورو تقريباً 1.08 دولار
    'eur': 1.08,
    'دولار': 1.0,
    'usd': 1.0,
    'usdt': 1.0
...
client.on('ready', () => {
    console.log(`Logged in as ${client.user.tag}!`); });  // ترحيب تلقائي بالأعضاء الجدد فقط عند دخولهم السيرفر client.on('guildMemberAdd', async member => {     const welcomeChannel = member.guild.systemChannel;     if (!welcomeChannel) return;     const msg = await welcomeChannel.send(`> أهلاً بك يا ${member} في سيرفر **${member.guild.name}**! نورتنا 🎉`);
...
client.on('messageCreate', async message => {
    ...
    // نظام الشراء المتقدم (مخطط الخطوات: اختيار الدفع -> كتابة العملة -> كتابة الكمية والتحويل)
    if (activePurchases.has(message.author.id)) {
        ...
        if (message.channel.id === purchaseData.channelId) {
            ...
            if (purchaseData.step === 'currency') {
                const currencyInput = message.content.trim().toLowerCase();
                purchaseData.currency = message.content.trim();
            ...
            // الخطوة 2: استقبال الكمية وحساب التحويل إلى الدولار
            if (purchaseData.step === 'amount') {
                ...
                const rate = exchangeRates[currKey] || 1.0; // إذا لم تكن معروفة، نعتبرها بقيمتها أو دولار افتراضي
                const convertedUSD = (amount * rate).toFixed(2);

                activePurchases.delete(message.author.id);
                await message.delete().catch(() => {});

                // حذف رسالة الطلب السابقة إن وجدت
    ...
    if (command === 'سيرفر') {
        ...
        const sEmbed = new EmbedBuilder()
            .setColor('#2b2d31')
            .setTitle(`📊 معلومات السيرفر: ${message.guild.name}`)             .setThumbnail(message.guild.iconURL({ dynamic: true }))             .addFields(                 { name: '👑 الأونر', value: `${owner.user.tag}`, inline: true },
                { name: '👥 عدد الأعضاء', value: `${message.guild.memberCount}`, inline: true },                 { name: '🚀 مستوى البوستات', value: `Level ${message.guild.premiumTier} (${message.guild.premiumSubscriptionCount} Boosts)`, inline: true },                 { name: '📅 عمر السيرفر', value: `<t:${Math.floor(message.guild.createdTimestamp / 1000)}:R>`, inline: true },
                { name: '🆔 آي دي السيرفر', value: `\`${message.guild.id}\``, inline: true },
                { name: '💬 عدد الرومات', value: `${message.guild.channels.cache.size}`, inline: true }             );         return message.reply({ embeds: [sEmbed] });     }      if (command === 'بروفايل' \vert{}\vert{} command === 'اي_دي') {         const target = message.mentions.users.first() \vert{}\vert{} message.author;         const member = message.guild.members.cache.get(target.id);         const embed = new EmbedBuilder()             .setColor('#2b2d31')             .setTitle(`👤 معلومات العضو: ${target.username}`)
